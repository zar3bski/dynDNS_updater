...
class TestExtractor(TestCase): 
    ...
    def test_(self): 
        with open("./tests/out/response-2.json") as payload:
            filtered = Extractor._filter_records(json.load(payload), "serial", 1609351415)
            self.assertTrue(len(list(filtered)),1)

    def test_2(self): 
        with open("./tests/out/response-2.json") as payload:
            filtered = Extractor.extract_first_field_value(json.load(payload), "serial", 1609351415, "expire")
